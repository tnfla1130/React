...
function LifeGood(props){
  ...
  /** 
  State생성 : 컴포넌트는 State의 상태가 변경될때마다 새롭게 렌더링된다. 
   */
  ...
  /** 
  앞에서 useEffect가 먼저 선언되었지만, 수명수기에서는 렌더링이 먼저 
  ...
  return (
    <div>
      ...
      <p>난수 : {myRandomNum}</p>
      <p>카운트 : {myCount}</p>
      {/* 버튼을 누를때마다 난수를 생성하거나 카운트를 증가시켜 State를 
      변경한다. State의 변경은 새로운 렌더링으로 이어진다.  */}
