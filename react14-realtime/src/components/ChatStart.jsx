...
const ChatStart = () => {
  ...
  */
  ...
  open() 함수를 이용해서 채팅창을 팝업으로 오픈한다. 
    형식] open(팝업창의 요청 URL, 창의 이름, 창의 속성);
      두번째 인수인 '창의이름'에 이름을 부여하면, 새로운 창을 열었을때
      항상 같은 위치에서 열리게된다. 즉 새로운 창을 여룻없으므로 여기서는
      '창의이름'을 부여하지 않아야 한다. 
  */
  const openChatWin = () => {
    /*
  ...
  return(<>
    <div className="App">
      ...
      {/* input에 앞에서 생성한 Ref 변수를 추가하여 DOM에 접근한다. 
      이렇게 하면 getElementXX와 같은 함수 없이 쉽게 접근할 수 있다.  */}
      {/* 대화방의 아이디는 room1로 고정한다. 단 필요하다면 변경하거나
      입력하면 된다.  */}
      방명 : <input type="text" name="roomId" value="room1" readOnly ref={refRoom}/><br />
